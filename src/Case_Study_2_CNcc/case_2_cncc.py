...
def beta_acq(df):
    ...
    mask = acquisition_df.apply(lambda row: (row['t'], row['p'], row['cat'], row['arbr'], row['pyrr']) in existing_tuples, axis=1)
    ...
    mean_dimension_t = acquisition_df.loc[(acquisition_df['p'] == max_ei_row['p']) & (acquisition_df['cat'] == max_ei_row['cat']) & (acquisition_df['arbr'] == max_ei_row['arbr']) & (acquisition_df['pyrr'] == max_ei_row['pyrr']), 'value_ucb'].mean()
    mean_dimension_p = acquisition_df.loc[(acquisition_df['t'] == max_ei_row['t']) & (acquisition_df['cat'] == max_ei_row['cat']) & (acquisition_df['arbr'] == max_ei_row['arbr']) & (acquisition_df['pyrr'] == max_ei_row['pyrr']), 'value_ucb'].mean()
    mean_dimension_cat = acquisition_df.loc[(acquisition_df['t'] == max_ei_row['t']) & (acquisition_df['p'] == max_ei_row['p']) & (acquisition_df['arbr'] == max_ei_row['arbr']) & (acquisition_df['pyrr'] == max_ei_row['pyrr']), 'value_ucb'].mean()
    mean_dimension_arbr = acquisition_df.loc[(acquisition_df['t'] == max_ei_row['t']) & (acquisition_df['p'] == max_ei_row['p']) & (acquisition_df['cat'] == max_ei_row['cat']) & (acquisition_df['pyrr'] == max_ei_row['pyrr']), 'value_ucb'].mean()
    mean_dimension_pyrr = acquisition_df.loc[(acquisition_df['t'] == max_ei_row['t']) & (acquisition_df['p'] == max_ei_row['p']) & (acquisition_df['cat'] == max_ei_row['cat']) & (acquisition_df['arbr'] == max_ei_row['arbr']), 'value_ucb'].mean()
...
if mode == "initialization":
    ...
    arbr_selected = random.sample(arbr_items, n_init) if n_init <= len(arbr_items) else random.choices(arbr_items, k=n_init)
    pyrr_selected = random.sample(pyrr_items, n_init) if n_init <= len(pyrr_items) else random.choices(pyrr_items, k=n_init)
    ...
    df['yields'] = df.apply(lambda row: 0, axis=1)
    ...
    cat = snar_data['cat'].iloc[:-5].values
    arbr = snar_data['arbr'].iloc[:-5].values
    pyrr = snar_data['pyrr'].iloc[:-5].values
    yields = snar_data['yields'].iloc[:-5].values
    ...
    results = []
    for t_val in domain_vars[0]['items']:
        for p_val in domain_vars[1]['items']:
            for cat_val in domain_vars[2]['items']:
                for arbr_val in domain_vars[3]['items']:
                    for pyrr_val in domain_vars[4]['items']:
                        x_raw = [t_val, p_val, cat_val, arbr_val, pyrr_val]
                        x_input = [opt.func_caller.get_processed_domain_point_from_raw(x_raw)]
                        mu, stdev = opt.gp.eval(x_input, uncert_form='std')
                        results.append([t_val, p_val, cat_val, arbr_val, pyrr_val, mu[0], stdev[0]])
    ...
    print('mu_stdev_extraction finished')
